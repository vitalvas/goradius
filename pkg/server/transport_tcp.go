...
func readRADIUSPacket(r io.Reader) ([]byte, error) {
	...
	header := make([]byte, packet.PacketHeaderLength)
	...
	// Validate length
	if length < packet.MinPacketLength {
		return nil, io.ErrUnexpectedEOF
	}
	if length > packet.MaxPacketLength {
		return nil, io.ErrUnexpectedEOF
	}

	// If packet is just header (no attributes), return it
	if length == packet.PacketHeaderLength {
		return header, nil
	}
	...
	copy(data, header)
	if _, err := io.ReadFull(r, data[packet.PacketHeaderLength:]); err != nil {
		return nil, err
	}
